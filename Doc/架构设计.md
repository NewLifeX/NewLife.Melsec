...
- 支持三种帧类型：3E 帧（Qna 兼容）、1E 帧（A 系列兼容）、4E 帧（iQ-R 系列，含序列号字段）
- 4E 帧在 3E 帧子头后增加 4 字节序列号（2B 序号 + 2B 保留），子头 5400h（请求）/ D400h（响应）
...
└── Protocols/
    ...
    ├── MC1EDeviceCode.cs           ← MC 1E 帧软元件代码枚举 [✅]
    ├── MCFrameType.cs              ← MC 帧类型枚举（3E/1E/4E）[✅]
...
| MC 新特性（UDP/串口/随机读/远程控制/4E帧/密码锁定/监视注册） | MCProtocolNewFeaturesTests | 含在 MC 总数内 |
| MC 类型转换 | MCTypeConversionTests | 含在 MC 总数内 |
| 尚未独立测试组件（SYS-3/SYS-4/FXL-9/MC-7/MC-11） | UntestedComponentsTests | 26 |
| SLMP 协议 | SLMPTests | 5 |
| HexHelper | HexHelperTests | 1 |
| **总计** | | **~216** |

> 2026-07-22 执行 `dotnet test XUnitTest`，全部通过（216/216），0 失败、0 跳过。
